        ...
        <header class="flex justify-between items-start mb-2">
            ...
            <div class="relative inline-flex" x-data="{ open: false }">
                <button
                    ...
                    :aria-expanded="open"
                >
                    <span class="sr-only">{{ __('dashboard.card_actions.menu') }}</span>
                ...
                >
                    <ul>
                        <li>
                            <a class="font-medium text-sm text-gray-600 dark:text-gray-300 hover:text-gray-800 dark:hover:text-gray-200 flex py-1 px-3" href="#0" @click="open = false" @focus="open = true" @focusout="open = false">{{ __('dashboard.card_actions.option_1') }}</a>
                        </li>
                        <li>
                            <a class="font-medium text-sm text-gray-600 dark:text-gray-300 hover:text-gray-800 dark:hover:text-gray-200 flex py-1 px-3" href="#0" @click="open = false" @focus="open = true" @focusout="open = false">{{ __('dashboard.card_actions.option_2') }}</a>
                        </li>
                        <li>
                            <a class="font-medium text-sm text-red-500 hover:text-red-600 flex py-1 px-3" href="#0" @click="open = false" @focus="open = true" @focusout="open = false">{{ __('dashboard.card_actions.remove') }}</a>
                        </li>
                    </ul>
                </div>
            </div>
        </header>
        <div class="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase mb-1">{{ __('dashboard.metrics.sales') }}</div>
