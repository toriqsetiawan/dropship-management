<x-app-layout>
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-2xl mx-auto">
        <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100 mb-6">{{ __('employee.create_title') }}</h1>
        <form action="{{ route('employees.store') }}" method="POST" class="bg-white dark:bg-gray-800 shadow rounded-lg p-6 space-y-6">
            @csrf
            <div>
            ...
            <div>
                ...
                <input type="number" name="salary" id="salary" class="form-input w-full" value="{{ old('salary') }}" min="0" required>
            ...
            <div>
                ...
                <select name="type" id="type" class="form-select w-full" required>
                    ...
                    <option value="fulltime" {{ old('type') == 'fulltime' ? 'selected' : '' }}>{{ __('employee.types.fulltime') }}</option>
                    <option value="parttime" {{ old('type') == 'parttime' ? 'selected' : '' }}>{{ __('employee.types.parttime') }}</option>
                </select>
            ...
            <div class="flex justify-end gap-2">
                ...
                <button type="submit" class="btn bg-indigo-500 hover:bg-indigo-600 text-white">{{ __('employee.actions.save') }}</button>
            </div>
        </form>
    </div>
</x-app-layout>
