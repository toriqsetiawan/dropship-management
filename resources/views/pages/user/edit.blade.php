                ...
                <div class="p-6 lg:p-8">
                    ...
                    <form action="{{ route('users.update', $user) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            ...
                            <!-- Role -->
                            <div x-data="{ role: '{{ old('role', $user->role->name) }}' }">
                                <x-label for="role" :value="__('user.fields.role')" />
                                <select id="role" name="role" required class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm uppercase" x-model="role">
                                    <option value="">{{ __('Select Role') }}</option>
                                    @foreach($roles as $roleObj)
                                        <option value="{{ $roleObj->name }}" {{ old('role', $user->role->name) == $roleObj->name ? 'selected' : '' }}>
                                            {{ $roleObj->name }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <div x-show="role === 'reseller' || role === 'retail'" class="mt-4">
                                    ...
                                    <select name="parent_id" id="parent_id" class="form-select w-full">
                                        ...
                                        @foreach($distributors as $distributor)
                                            <option value="{{ $distributor->id }}" {{ old('parent_id', $user->current_team_id) == $distributor->id ? 'selected' : '' }}>
